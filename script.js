...
let text = document.getElementById("player-name");

function burnText() {
    text.style.animation = "burnEffectOut 2s forwards"; // Efek terbakar lebih lambat saat menghilang
    
    setTimeout(() => {
        text.style.opacity = "0"; // Hilang total
        setTimeout(() => {
            text.style.animation = "burnEffectIn 2s forwards"; // Efek terbakar lebih lambat saat muncul
            text.style.opacity = "1"; // Muncul lagi
        }, 1000); // Delay sebelum muncul lagi
    }, 2000); // Waktu untuk efek terbakar saat menghilang
}

// Ulangi efek setiap 6 detik agar lebih nyaman dilihat
setInterval(burnText, 6000);
